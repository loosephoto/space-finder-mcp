...
- v0.38.1 — **メディアのリンク行の見た目を統一し、複数の画像を返したときにリンクが1行に融合する問題を修正**: Markdown は同一段落内の単一改行をソフト改行として畳み込むため、`content` を単一改行で組むと caption とリンク／リンクどうしが **1行に融合**していた（1枚だけのときは段落が分かれて見えるので気付きにくい）。**リンク行の文言は `img_common.media_link_line(対象, kind=…)` が生成**するようにし、動詞を kind から機械的に決定（`画像を開く`/`生成した画像を開く`/`音声を開く`/`動画を開く`/`ファイルを開く`）＝全41呼び出しを統一形式へ（`動画を再生:` → `動画を開く:` 等。ゲートは `canonical_media_label()` で検査）。加えて全ツールの唯一の出口 `server._threaded` の `_delivered()` が `img_common.layout_media_links()`（冪等・リスト内の字下げは保持）で**リンク行の前後に空行**を入れるので、ツール側で入れ忘れても融合しない（キャッシュ共有の戻り値は書き換えず、変化があるときだけ `model_copy`）。`--media-links` は「リンク先行・形式の統一・空行区切り（整形後の配信形）・配信時の整形（`delivery_issues()`）・structuredContent」を検査し、対象を**メディアのリンク行を持つツール**へ拡大（`apod` / `eodashboard_detail` / `astronomy_weather` / `search_space_audio`・`search_space_videos`）。検証: `--media-links` 18ツール・問題0、`--stdio` 6/6、`--concurrency` 3/3、`--figures` 描画系9、`--offline`、`--dead-code` 0件、回帰テスト **228件 OK**（新規11件 `tests/test_media_layout.py`）、全62ツール exit 0（61 OK・1 ERROR は NASA DEMO_KEY の 400＝`apod`）。
