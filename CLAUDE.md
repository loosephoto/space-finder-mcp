...
宇宙・天文・地球観測の公開データを **58ツール**で横断検索する MCP サーバー（Python / uv 管理）です。NASA・ESA Copernicus・JAXA・ISRO・CSA・INPE・UK EO DataHub・CNSA・CelesTrak・JPL（DE421/SBDB/Horizons）・Wikidata・Open-Meteo などを統合し、衛星画像・軌道マップ・日食パネル・月齢マップを**画像で返しつつ**、LLM向けに構造化JSONも同時に返します。
...
検証済みの代替コマンド（いずれも 58ツールを返して起動します）:
...
uv run python scripts/check-tools.py                  # 全58ツール実呼び出し（数分）
...
3. `uv run python scripts/check-tools.py` で全58ツールが正常なことを確認
