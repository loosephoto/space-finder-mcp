...
**「彗星の見え方チャートを追加（`solar_system_now(view="apparition")`）— 距離・予想光度・太陽離角の時系列で観測の可否を示す」**（v0.35.0）。

- **☄️ 何を描くか** — 横軸＝UTC の日付で、**距離**（日心距離 r／地心距離 Δ）・**予想光度 m1**（JPL SBDB の全光度の式）・**太陽離角**の3段パネル。今日・近日点・地球最接近を縦線で示し、期間内の極値は数値ラベル付きの点で示します。`days`（既定180日）で長さを変えられ、**今日の30日前から**描くので直前に過ぎた近日点・最接近も見えます（1天体ずつ）。認証不要。
- **📐 位置は N 体解（座標系を揃えるのが要点）** — JPL Horizons の状態ベクトルを期間ぶん1リクエストで取得（`REF_PLANE=FRAME`＝ICRF）。**実測: Horizons の `ECLIPTIC`（黄道 J2000）と DE421 の地球位置を混ぜると Δ が 0.003 au（45万 km）ずれる**ため、両方を ICRF に統一しました（`DES=1P;` が多重登録で解決できないハレー彗星は `;CAP` を付けて解決＝実測）。Horizons が落ちたときだけ SBDB 要素の2体近似へ退避し、**精度差を数値で注記**します（実測: 169P/NEAT で N 体解と最大 0.0249 au・地球最接近の時刻が約1.2日ずれる）。
- **🧭 誤ったイベントを書かない** — 期間の端で最小のときは「近日点」「地球最接近」とは呼ばず「r 最小」「Δ 最小」に切り替え、注記に「真の極値はこの期間の外」を出します（ハレー彗星は 2061 年まで太陽へ近づき続けるので、どの期間でも最小は端に来る）。曲線や縦線に重なるラベルは描かず、描けなかったぶんは `figure.notes` に列挙。`verify` は**極値の座標にある曲線色の画素・ラベルの重なり・縦線3本の実在**を画素から測り直します。
- **🛠 ついでに直した既存バグ** — 未知の彗星名で `_comet_unknown_hint` が未定義のため **NameError がツールの外へ漏れていた**（規約1違反）のを修正し、指定できる名前の例を返すようにしました。
- **🧪 検証** — `--figures`（**追加経路 21**・うち `apparition` の3経路＝通常・期間の端・双曲線。全経路 `verify.ok=true`）／`--media-links`（**14ツール・問題0**。`apparition` の経路も追加）／`--dead-code`（0件）／`--fuzz`／`--stdio`／`--concurrency`／`--fonts`（すべて exit 0）／回帰テスト **141件 OK**（新規6件: 周期に 1e99 の番兵が来たときの OverflowError、e=0.5/1.0/1.2 で r=q、`view="apparition"` の引数エラー、未知の彗星名で NameError が漏れていた件）。

登録ツールは **56本**（既存ツールへの `view` 追加なので本数は変わりません）。
