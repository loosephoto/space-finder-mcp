...
- **🌕 月面の新クレーター地点図** — `planetary_orbiter_track(body="moon", sites="newcrater")` で自然衝突の最新例マクゲッチンと、人工物衝突の最新例ファルコン9上段を同じ地図に表示します。地点種別ごとに「形成／衝突」を表示し、`sites="all"` の混在図は `figure.kind="site_map"` として区別します。
