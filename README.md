...
| 種別 | 出る場所 | 形 |
|------|----------|-----|
| **生成画像**（描画系8ツール） | `content` の**先頭行** | `🖼️ [生成した画像を開く: 対象](file:///…) ｜ 保存先: \`…\`` |
| **検索した画像**（`search_space_images`） | 各項目の説明の直後 | `🖼️ [画像を開く: タイトル](URL)` |
| **検索した音声**（`search_space_audio`） | 各項目の直後 | `🎧 [音声を開く: タイトル](URL)` |
| **検索した動画**（`search_space_videos`） | 各項目の直後 | `🎬 [動画を開く: タイトル](URL)` ＋ `🖼️ [画像を開く: タイトル のポスター](URL)` |
| **気象衛星の実画像**（`weather_satellite_now`） | `content` の**先頭行** | `🖼️ [生成した画像を開く: ひまわり9号 最新画像](URL または file:///…)` ＋ 保存先 |
| **気象庁の雨雲・降水画像**（`astronomy_weather`） | 各画像の直前行 | `🖼️ [画像を開く: ラベル](気象庁のページURL)` |
| **EO Dashboard のサムネイル**（`eodashboard_detail`） | 説明の直後・画像の直前 | `🖼️ [画像を開く: タイトル のサムネイル](URL)` |
| **APOD の画像URL**（`apod`） | タイトルの直後 | `🖼️ [画像を開く: タイトル](URL)`（動画の日は `🎬 [動画を開く: タイトル]`） |
...
- **機械的に検査します** — `scripts/check-tools.py --media-links`（画像ブロックより前にアイコン付きリンクが無い／リンクの文言が統一形式でない／リンク行が空行で区切られていない／`structuredContent` にURL・保存パスが無い／`image_path` のファイルが存在しない／配信時（`server._threaded`）に整形されていない場合は exit 1。実測: メディアを返す全ツール OK）。生成系の docstring には「回答時はこのリンクをそのまま提示してください」と明記しています。
...
**「全球等角図ベースマップの取得経路を拡張し、経度原点の違いで別の場所を指す問題を修正」**（v0.38.2）。

- **🌍 気体惑星に全球図（Hubble OPAL）** — 木星・土星・天王星・海王星を緯度経度グリッドから **Hubble OPAL の全球等角図**（MAST/STScI 公開・DOI 10.17909/T9G593）へ。SL9 衝突地点図も木星の全球図の上に描けるようになりました（**その年の大気の模様**なので観測年を注記に出し、1994年当時の模様でも衝突痕そのものが写っているのでもない旨を明示）。
- **🪐 冥王星・ベンヌにも全球図** — 冥王星＝New Horizons 全球カラー図、ベンヌ＝OSIRIS-REx 全球モザイクを追加（`trek`／`image`／`graticule` の3方式を天体ごとに自動選択。地点が登録されていない天体も `sites="map"` で全球図を返す＝対応 **18天体**）。
- **🧭 経度原点の宣言（`left_edge_lon`）** — USGS/PDS の全球モザイクと Hubble OPAL は**画像の左端が東経 0°**（実測: 冥王星の `.lbl` は `CenterLongitude=180`、OPAL の readme は "left edge = 0 System III W"）で、地点マーカー図の投影（左端 西経 180°）とは **180° ずれる**。そのまま描くと**別の場所を指す**ため `ImageChops.offset` で**半周回してから**描き、回した角度と px を注記に数値で出します（金星の Magellan 図は左端 -180° なので回転不要）。配置の正しさは大赤斑で実測確認（2時期の位置差 195° ≒ 公表ドリフト 193°、生成図の大赤斑は東経 38.1°＝元図からの期待値と一致）。
- **🧪 検証** — 回帰テスト **236件 OK**（新規8件 `tests/test_planetary_basemap_roll.py`＝回転の要否と原点の規約）／`--figures`（描画系9・追加経路36）／`--media-links`／`--offline`／`--stdio`／`--concurrency`／`--dead-code`（0件）／全62ツール exit 0。

登録ツールは **62本**。

### 以前の更新

- **v0.38.1** — **メディアのリンク行の見た目を統一し、複数の画像を返したときにリンクが1行に融合する問題を修正**（原因は Markdown のソフト改行。リンク文言を `img_common.media_link_line` が生成し、配信時に `layout_media_links` で空行を保証。`--media-links` を強化）。
- **v0.38.0** — **天体カタログ（SIMBAD / VizieR）・系外惑星（NASA Exoplanet Archive）・みちびき（QZSS）の4ツールを追加（58→62ツール）**。月面の新クレーター地点図（マクゲッチン／ファルコン9上段）も追加。
...
| `planetary_orbiter_track` | 任意の天体（月・火星・水星・タイタン等）を周回する探査機の現在位置と軌道トレイルを、その天体の地図にプロットした画像を返す。JPL Horizons の状態ベクトルを IAU 自転モデルで天体固定座標（緯度経度・高度）に変換し、NASA Trek の等角図法地図に重ねる。`span_deg=360`で天体全面表示にも対応。**過去機（かぐや等）は運用終了を案内し、落点が公表されている機体は落点を地点マーカーで描いた地図（`figure.kind="impact_site_map"`）を返す**。`sites="apollo"`（または `"apollo11"`〜`"apollo17"`, `"all"`, `"newcrater"`）で **地点マーカー図**（`figure.kind="landing_site_map"`／木星は `impact_site_map`）も返す。**18天体**（月・火星・水星・金星・タイタン・ベスタ・ケレス・イオ・エウロパ・ガニメデ・カリスト・木星・土星・天王星・海王星・冥王星・ベンヌ・リュウグウ）に対応し、**全球等角図がある天体は地形画像の上に、無い天体（リュウグウ）は緯度経度グリッドの上に**公表座標を描く（全球図は NASA Trek のタイル／USGS の全球モザイク／**Hubble OPAL の全球図**（気体惑星）／New Horizons・OSIRIS-REx のモザイクを天体ごとに自動選択） | JPL Horizons + NASA Trek / USGS Astrogeology / Hubble OPAL (STScI) / NASA NSSDC / PDS | 不要 |
...
- **新しくできたクレーター（`sites="newcrater"`）**: 月面で確認された「新しいクレーター」2点を返します。**マクゲッチン**（自然衝突・2024-04-11〜05-22 に形成、直径 222 m・深さ 43 m、1.3536°N 67.1765°E。LROC が 2025-10-24 に発見、IAU 命名 2026-05-04。この規模の衝突は月で平均 132 年に1回）と、**ファルコン9上段の衝突痕**（人工物・2026-08-05 06:35 UTC、直径 18 m・深さ 3 m 未満、19.4759°N 266.7138°E＝西経 93.2862°）。個別指定は `"mcgetchin"` / `"falcon9"`。`figure.kind` は `impact_site_map` で、注記には「記録上いちばん新しいのは人工物の衝突（ファルコン9上段）、自然衝突ではマクゲッチンが最新」を**数値から生成**し、0〜360°E で公表された経度は -180〜180° に折り返して描いた旨と等価な西経も出します。
- **地点種別と座標の返却**: `sites="all"` は登録済み地点を種別混在で返すため、表示行ごとに着陸・形成・衝突を区別します。図の `kind` は着陸地点のみ=`landing_site_map`、クレーター／衝突地点のみ=`impact_site_map`、着陸地点と衝突系の混在=`site_map` です。構造化データの各地点には `kind` と `event` が入り、投影用経度は -180〜180° に正規化します（公表値が 266°E の場合は本文と注記に西経換算も併記）。
- **対応天体（`sites=` の地点マーカー図）**: `MAP_BODIES` の18天体（月・火星・水星・金星・タイタン・ベスタ・ケレス・イオ・エウロパ・ガニメデ・カリスト・木星・土星・天王星・海王星・冥王星・ベンヌ・リュウグウ）。**ベースマップは3種類を天体ごとに自動選択**します。地点が登録されていない天体でも `sites="map"` で全球図そのものを返します。
  1. **`trek`** … NASA Trek の全球等角図タイル（層IDを検証済みの5天体: 月・火星・水星・ベスタ・ケレス）
  2. **`image`** … **1枚の全球等角図**（金星＝USGS/NASA Magellan 合成図、タイタン＝NASA Cassini 全球図、イオ/エウロパ/ガニメデ/カリスト＝USGS/NASA のモザイク、**木星・土星・天王星・海王星＝Hubble OPAL の全球図**（MAST/STScI 公開・DOI 10.17909/T9G593。**その年の大気の模様**なので観測日を注記に出す）、**冥王星＝New Horizons 全球カラー図、ベンヌ＝OSIRIS-REx 全球モザイク**）。**2:1 でない画像はアスペクト比を補正して描画**（補正率と元の AR を注記に明示）
  3. **`graticule`** … 全球等角図が確認できない天体（リュウグウ。USGS の全球モザイク倉庫に該当キー 0 件・NASA Trek のベースマップ層を配る索引サービスは HTTP 500＝実測）は緯度経度グリッド。注記に「地形画像は無い」と明示
  **全球等角図の経度原点は天体ごとに違う**ので、`image` 設定は画像の左端の経度を `left_edge_lon` で自己申告し、左端が東経 0° の図（USGS/PDS の全球モザイク・Hubble OPAL）は**横に半周（180°）回して**描きます（回した角度・px・元の px を注記に数値で出す。実測: 冥王星の `.lbl` は CenterLongitude=180、OPAL の readme は「left edge = 0 System III W」、金星の Magellan 図は左端 -180°＝回転不要）。
