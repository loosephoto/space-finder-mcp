...
**描画系ツールが「図をどう描いたか」を自己申告するようになり、その自己申告を検査する仕組みが実バグを見つけました**（v0.26.0）。

- **`structuredContent.figure`（`schema: "figure/1"`）を描画系6ツールに導入** — 視点(`view`)・主天体の置き方(`primary`: 楕円は**焦点**であって中心ではない)・縮尺(`scale`)・円錐曲線(`conic`: 楕円/双曲線/放物線)・注記(`notes`)・自己検証(`verify`)・`caption` を返します。注記は**数値から生成**（手書きしない）ので、図と文章が食い違いません。LLM はピクセルから描画規約を推測できないため（高離心率の軌道を「主天体の周りを回る円」と説明してしまう等）の対策です。`content` にも同じ注記を出し、docstring には「`figure.notes` は要約せず引用する」と明記しています。
- **彗星の軌道面ビューを追加（`solar_system_now(view="comet_orbit")`）** — 彗星を**その軌道面を真横から見た図**で描き、太陽＝焦点、近日点・遠日点、逆行（i>90°）を注記します。C/2023 A3 のような **e>1（双曲線）** は「閉じない軌道＝遠日点なし」として枝で描き分けます。
- **`scripts/check-tools.py --figures` を新設** — 描画系ツールの `figure.notes` が空、または `verify.ok` が偽なら exit 1。`view="comet_orbit"` のような**既定引数では通らない経路**や、可視の日食も明示的に叩きます。
- **`verify` は画素から描画を測り直します** — 彗星・地上軌道図は描いた曲線から近点/遠点を逆算、日食パネルは PA 軸の明部長から食分を逆算、火星ローバーはマーカー画素から緯度経度を逆算、衛星オーバーレイは az/alt から画素位置を再計算して赤マーカーを確認。**この自己検証が実バグ3件を検出しました**：
  1. 日食図で**隣のパネルの月が太陽を覆っていた**（パネル毎にレイヤーを作って切り貼りするよう修正）
  2. 日食図で**「最大食分」ラベルが中央パネル固定**だった（実際の最大のパネルに付けるよう修正）
  3. 衛星オーバーレイで**衛星ラベル枠が他衛星のマーカーを隠していた**（全マーカーを描いてから衝突回避配置するよう修正）
- **日食の正しさを2点修正** — (a) 食が現地の日付をまたぐ場合に**走査窓が食を切り詰めて最大食分を過小報告**していた（前後6時間まで広げて計算）。 (b) **太陽が地平線下の「見えない日食」を返していた**（高度>0 の時間帯だけを描き、全日食が地平線下なら「見えません」と明示。例: 東京2028-01-26は最大高度 −67°）。v0.26.0 の `solar_eclipse_series` は東京の既定呼び出しで「今後約2年に見える日食なし」と正しく答え、`date="2030-06-01"` を案内します。
- **検証** — 幾何の独立検算: 2024-04-08 の米国日食で Dallas 1.014（皆既）・New York 0.910（部分・実測一致）、東京2035-09-02 は 0.993（99%部分食）。日食パネルの画素→食分の再計算は誤差 0.004 以内。`check-tools.py` の全モード（全45ツール／`--offline`／`--dead-code`／`--fuzz`／`--figures`）で異常0。
