...
**「メディアのリンク行の見た目を統一し、複数の画像を返したときにリンクが1行に融合する問題を修正」**（v0.38.1）。

- **📐 原因は Markdown のソフト改行の畳み込み** — `content` を単一改行で組むと、描画側は同一段落へ畳み込み、**caption とリンク／リンクどうしが1行に融合**します（1枚だけ返すときは段落が分かれて見えるので気付きにくく、複数返したときだけ症状が出る）。
- **🖼️ 文言を1か所で生成（見た目の統一）** — リンク行は `img_common.media_link_line(対象, kind=…)` が作ります。動詞は `kind` から機械的に決まる（`画像を開く` / `生成した画像を開く` / `音声を開く` / `動画を開く` / `ファイルを開く`）ため、ツール側で動詞を書かない限り表記は揃います。全41呼び出しを統一形式へ変更（`動画を再生:` → `動画を開く:`、`サムネイル画像を開く: X` → `画像を開く: X のサムネイル`、`生成した画像を開く（X）` → `生成した画像を開く: X`）。ゲートは `img_common.canonical_media_label()` で検査します。
- **🧱 入れ忘れを不可能に（配信時に整形）** — 全ツールの唯一の出口 `server._threaded` の `_delivered()` が `img_common.layout_media_links()`（冪等・リスト内の字下げは保持）で**リンク行の前後に空行**を入れます。キャッシュ共有され得る戻り値は書き換えず、変化があるときだけ `model_copy` で新しい結果を組み立てます。
- **🔍 ゲートを強化** — `--media-links` に「リンク形式の統一」「空行区切り（**整形後＝配信される形**で判定）」「配信時の整形（`delivery_issues()` が `_threaded` に融合した結果を流して検証）」を追加し、検査対象も「メディアのリンク行を持つツール」へ拡大しました（これまで対象外だった `apod` / `eodashboard_detail` / `astronomy_weather` / `search_space_audio`・`search_space_videos` が入ります）。
- **🧪 検証** — 実経路（`search_space_images`）の `content` でリンクが独立した段落になることを確認。`--media-links`（18ツール・問題0）／回帰テスト **228件 OK**（新規11件 `tests/test_media_layout.py`）／`--stdio`（6/6）／`--concurrency`（3/3）／`--figures`（描画系9・追加経路36）／`--offline`／`--dead-code`（0件）／全62ツール実呼び出し（exit 0・61 OK・1 ERROR は既知の外部要因＝NASA DEMO_KEY の 400（`apod`））。

登録ツールは **62本**。

### 以前の更新

- **v0.38.0** — **天体カタログ（SIMBAD / VizieR）・系外惑星（NASA Exoplanet Archive）・みちびき（QZSS）の4ツールを追加（58→62ツール）**: `object_lookup`（和名可→主名/種別/スペクトル型/年周視差→pc・光年/等級、`neighbors` で周辺天体を角距離順）・`catalog_search`（VizieR 星表の円錐検索。半径は0.1分が下限、未知の星表は候補提示で停止）・`exoplanet_search`（発見方法・距離・発見年・地球類似条件で検索。和名は英字名へ展開）・`qzss_status`（超速報 SP3 から各機の現在位置・東京からの仰角方位。提供元明記の**非商用利用のみ**）。JVO は機械可読窓口が無く（404）、OACAPI は TLS 証明書失効のため見送り。月面の新クレーター地点図（マクゲッチン／ファルコン9上段）も追加。
...
| `planetary_orbiter_track` | 任意の天体（月・火星・水星・タイタン等）を周回する探査機の現在位置と軌道トレイルを、その天体の地図にプロットした画像を返す。JPL Horizons の状態ベクトルを IAU 自転モデルで天体固定座標（緯度経度・高度）に変換し、NASA Trek の等角図法地図に重ねる。`span_deg=360`で天体全面表示にも対応。**過去機（かぐや等）は運用終了を案内し、落点が公表されている機体は落点を地点マーカーで描いた地図（`figure.kind="impact_site_map"`）を返す**。`sites="apollo"`（または `"apollo11"`〜`"apollo17"`, `"all"`, `"newcrater"`）で **地点マーカー図**（`figure.kind="landing_site_map"`／木星は `impact_site_map`）も返す。**全球等角図がある天体（月・火星・水星・タイタン・ベスタ・ケレス）は地形画像の上に、無い天体（木星・土星・冥王星・金星・ガリレオ衛星など）は緯度経度グリッドの上に**公表座標を描く | JPL Horizons + NASA Trek / NASA NSSDC / PDS | 不要 |
