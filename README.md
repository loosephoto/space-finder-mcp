...
## 🆕 直近の更新内容（v0.20.0 → v0.22.0）

**太陽系俯瞰図・日食時系列パネル・任意の観測地対応を追加**（v0.20.0 → v0.22.0）。

- **`solar_system_now`**: 太陽を中心とした太陽系の惑星・小惑星・探査機・彗星の現在位置俯瞰図を返す。惑星は JPL DE421 + Skyfield、任意の小惑星は JPL SBDB 軌道要素のケプラー伝播、遠方探査機は JPL Horizons、彗星は周期をSBDB・C/彗星をHorizonsで計算。内惑星から百数十AUまで対数縮尺で自動拡張し描画。認証不要。
- **`solar_eclipse_series`**: 指定した観測地・日付の**日食（太陽が月に欠ける過程）を時系列パネルで画像化**。太陽と月の視位置を JPL DE421 + Skyfield で実測計算し、月の位置角(PA)を反映して、食の始まり〜最大〜終わりを **7枚のパネル**に合成。`date` を省略すると「これから起こる次の日食」を新月の天文計算で絞り込みつつ自動検索（例: 2028-01-26 部分食・食分0.63）。`max_magnitude=True` で最大食のみの単一画像も可。食分から皆既/金環/部分を自動判定。**認証不要**。
- 既存の `solar_system_now` / `sky_map_with_satellites` と同じ Skyfield + Pillow 画像合成基盤を活用。食の進行・種別・最大食分・現地時刻をパネル毎に明記。
- **任意の観測地に対応**: 日食・ISS/星空・天体位置の `place` 引数で、東京に限らず**世界中の任意の地名**（例 パリ・ベルリン・ケープタウン・グリニッジ天文台・昭和基地）をジオコーディングして描画。日本語→英語変換に加え、複合地名・施設/天文台名・南極基地は **Open-Meteo + Nominatim(OSM)** の2段フォールバックで解決。日食の時刻は現地タイムゾーン（DST込み）で表示。

登録ツールは **43本**。認証不要のツールが大半です。
...
登録ツールは **43本**（他国の宇宙機関データ 15本＋火星探査ローバー 2本＋天文観測(ESO/CADC/ALMA) 3本＋電波望遠鏡(TART) 1本＋天文ニュース 1本＋天体観測用天気 1本＋天体位置・星座 1本＋星図合成 1本＋太陽系俯瞰 1本＋日食時系列 1本＋NASA POWER気候 1本＋EO Dashboard 2本＋宇宙天気 1本＋AWS STAC 2本＋ISS位置 1本＋WMO OSCAR 1本＋中国/ロシア打ち上げ・天宮 3本＋メディア/逆引き 4本）。すべて動作検証済みです。
