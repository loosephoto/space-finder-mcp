...
**NASA API の呼び出し枠を「投げる前に」管理し、探査機位置の再取得をなくしました**（v0.25.2）。

- **`nasa_budget.py` を新設（DEMO_KEY の予算管理）** — `apod`・`neo_today`・`space_weather`(DONKI) が共有する api.nasa.gov の枠（DEMO_KEY: 1時間30リクエスト/IP）をサーバー側で数え、使い切っていたら **HTTP を出さずに** 回復までの目安を返します。実際に 429 を受けた場合は **`Retry-After` を尊重**して待機し、同じ 429 を繰り返し踏みに行きません（従来は回復まで毎回 429 を発生させていました）。エラー応答には `budget`（上限・直近1時間の使用数・残り）を添えます。
- **探査機・彗星の Horizons 取得を分単位キーでキャッシュ** — `solar_system_now` は `when` 省略時に毎回現在時刻で JPL Horizons を呼んでいたため、画像を作り直すたびに外部APIを叩いていました。時刻を**分単位に丸めたキー**でキャッシュし、同一分内の再呼び出しはネットワークへ出ません（実測: 1回目 2.15s/1リクエスト → 2回目 **1.16s/0リクエスト**）。SBDB の軌道要素も同様に24時間キャッシュします（探査機は1分で数kmしか動かず、地図上は同一位置）。
- **CelesTrak の 403 を案内文付きに正規化** — 短時間に多数アクセスすると CelesTrak は IP 単位で遮断し、生の `403 Client Error` では原因が分かりませんでした。「一時的に拒否されました（数分〜しばらく待つと解除）」と返します。
- **検証** — `scripts/check-tools.py` の全モード（全45ツール／`--offline`／`--dead-code`／`--fuzz`）で異常0を確認。

登録ツールは **45本**。認証不要のツールが大半です。

### 以前の更新

- **v0.25.1** — 引数の防御的数値変換（`input_utils`・51箇所）で例外漏れを解消、`uk_stac_search` のキーワード検索修正、Claude Code / Codex インストール対応、リポジトリ直下 `.env` 対応、検証ゲート `scripts/check-tools.py` 同梱。
- **v0.25.0** — 任意天体の周回機・ローバー位置マップ（`planetary_orbiter_track` / `planetary_rover_location_map`）。`span_deg=360` で天体全面表示、`step` で秒単位の高精度トレイル。
