...
描画系ツール（`solar_system_now` / `sat_ground_track` / `planetary_orbiter_track` / `planetary_rover_location_map` / `sky_map_with_satellites` / `solar_eclipse_series`）は、画像を返すだけでなく **`structuredContent.figure`（`schema: "figure/1"`）** に「その図をどう描いたか」を自己申告します。LLM はピクセルから描画規約を推測できないため（高離心率の軌道を「主天体の周りを回る円」と説明してしまう等）、注記をデータとして渡す設計です。
...
- **`verify`** — 図の自己検証（描いた画素から測った近点／遠点距離、ラベルの線被り画素数）
...
| 種別 | 出る場所 | 形 |
|------|----------|-----|
| **生成画像**（描画系6ツール） | `content` の**先頭行** | `🖼️ [生成した画像を開く（…）](file:///…) ｜ 保存先: \`…\`` |
...
**「どのクライアントでも見える・どの表記でも引ける」を揃えた回**（v0.27.0）。メディアのリンク先出し、天体名・衛星名の解決を共通基盤へ集約、ALMA の取得項目拡張、両エンジンの配色一本化を行いました。

- **🔗 メディア本体より前にアイコン付きリンク** — `ImageContent` を描画しない CLI/Android 系ハーネス（codex / opencode など）では画像が無言で落ちるため、`content` の**先頭**に `🖼️ [生成した画像を開く](file:///…)` を出します。生成画像は `%LOCALAPPDATA%\Temp\space_finder_mcp\out` に保存（`structuredContent.image_path` も追加、最新200件で自動整理）。検索系は各項目の直前に `🖼️/🎧/🎬 [◯◯を開く: タイトル](URL)` を出し、URL 内の空白・括弧はパーセントエンコード（NASA の動画URLは空白入りで途中で切れるため）。
- **🧭 名前解決の共通フォールバック（`name_common.py` 新設）** — 内蔵テーブル → 表記ゆれ（空白/アンダースコア/連結/大小）→ **和名→英語名（103キー）** → **Sesame/CDS で名前→座標**（SIMBAD/NED 横断・認証不要）→ 候補提示して停止（推測しない）。実測で解消した例: `cadc_observations("M104")` の即エラー、`satellite_status(query="ひまわり")` と `sat_tle(name="ひので")` の0件、`alma_search(object_name="HL Tau", band=7)` の0件。現役機は CelesTrak `GROUP=active` と突合した NORAD ID を `WELL_KNOWN` に登録し、**和名だけでオフライン解決（衛星系キー 21/61）**、番号なしファミリー名（ひまわり／だいち）は候補を提示します。
- **📡 `alma_search` 拡張** — 表記ゆれの自動吸収、`product_type`（cube/image）・`public_only`・`with_products`（datalink で校正済み tar・生データ ASDM・README・パイプライン製品をリンク表示）を追加。返却にデータ種別・空間分解能・露出時間・可視降水量(pwv)・QA2 合否・PI・関連論文を追加しました。
- **🎨 配色の一本化** — `img_common.BODY_COLORS` / `SYMBOL_COLORS` を新設し、`sky_map_with_satellites`（simple/accurate）と `solar_system_now` が同一の指定色を共有。**matplotlib 版が全惑星を同じ薄青で描いていた不具合**を修正し、土星の環・木星の縞・火星の極冠・色コード付き凡例を simple 版と一致させました（軌道予測の破線も3分刻み→1分刻みで可視弧を正しく描画）。
- **🛡 安全性・堅牢性の修正2件** — (a) CelesTrak がリスト以外の JSON を返すと `sat_tle` から**例外が漏れていた**（`tuple(dict)` がキー列になる）のを正規化。(b) エラー文字列が URL ごと `api_key=<値>` を含み `structuredContent.error` に**API キーが露出**していたのを `nasa_budget.redact()` で伏せ字化。
- **検証** — 全45ツール実呼び出し（`OK 41 / ERROR_RESULT 4`、内訳は DEMO_KEY の枠による429×3と「東京は800日に日食なし」で全て想定内）／`--dead-code` 0件／`--fuzz` 252組合せ・例外漏れ0／`--offline` exit 0／`--figures` exit 0。
...
- **彗星の軌道面ビュー（`view="comet_orbit"`）**: `comet` と併せて指定すると、その彗星の軌道を**彗星自身の軌道面を真横から見た図**で返します。太陽は**円錐曲線の焦点**（楕円の中心ではない）に置き、近日点・遠日点・現在位置・日心距離目盛(AU)を描画します。**e≥1 の C/彗星**（例: 紫金山・アトラス＝SBDB の a が負）は**閉じない双曲線の枝**として近日点から有限距離までを描きます（楕円として描くと `a(1+e)` が負になり破綻するため）。ハレー彗星（e=0.968・逆行 i=162°）のような極端な楕円でも、**焦点と中心のズレ a·e** が図と `figure.notes` の両方に出ます。
...
{"comet": "1P/Halley", "e": 0.967936, "q_au": 0.5749, "typ": "sbdb",
 "figure": {"schema": "figure/1", "kind": "orbit_plane",
            ...
            "verify": {"periapsis_error_pct": 7.13, "apoapsis_error_pct": 0.04,
                       "label_overlap_px": 0, "ok": true}}}  // view="comet_orbit" の応答例
```
...
- `date` 省略時は**「これから起こる次の日食」を自動検索**（Skyfield almanac で新月を列挙し、食のありうる新月だけ高速絞り込み → 精密計算）。例: 東京の次に**見える**食は **2030-06-01 部分日食（食分0.80・太陽高度+19°）** で、既定の約2年探索では見つからないため `date="2030-06-01"` を案内します。
