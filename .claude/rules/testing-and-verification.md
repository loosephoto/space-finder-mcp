...
## 変更内容ごとの追加確認
- **描画系を変更したとき**: `uv run python scripts/check-tools.py --figures` で figure/1（注記・caption・`verify.ok`）を検査する。`verify.marker_pixels` / `panel_overlaps_marker` も見る（情報パネルが現在位置マーカーを隠していないか＝`surface_map.panel_placement`）。
